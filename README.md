...
A shared collection of agent **skills** that work across [Claude Code](https://code.claude.com/docs/en/skills), [Cursor](https://cursor.com/docs/skills), [CodeBuddy](https://www.codebuddy.ai/docs/cli/skills), and [Codex](https://developers.openai.com/codex/skills).

## Why this layout

All four tools share the **same skill format** — a directory containing a `SKILL.md` file with YAML frontmatter (`name`, `description`) plus optional `scripts/`, `references/`, and `assets/` folders. The only thing that differs is *which directory* each tool scans:
...
Instead of duplicating skills into four places, this repo keeps a single canonical source and exposes it to every tool via symlinks:
...
Because every tool points at the same `skills/` folder, the new skill is immediately available to Claude Code, Cursor, CodeBuddy, and Codex — no per-tool copying.
...
```bash
# default: symlink, user scope (~/.claude, ~/.cursor, ~/.codebuddy, ~/.agents), all agents
./install.sh
...
`install.sh` also propagates the repo's `AGENTS.md` to each tool's instructions file, so all four agents share one source of truth. Use `--no-skills` / `--no-agents-md` to opt out of either part.
...
Notes:
- **Project scope** only creates the `CLAUDE.md` bridge for Claude Code; Cursor, CodeBuddy, and Codex already read `./AGENTS.md` natively.
...
```bash
git clone <this-repo> ~/agent-plugins
cd ~/agent-plugins
