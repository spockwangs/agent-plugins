...
#        Cursor / CodeBuddy / Codex read AGENTS.md natively.
#      - user scope: ~/.claude/CLAUDE.md, ~/.cursor/rules/agents.mdc,
#        ~/.codebuddy/CODEBUDDY.md, ~/.codex/AGENTS.md
#
# Usage:
...
ALL_AGENTS="claude cursor codebuddy codex"
...
MARKER='<!-- installed by agent-plugins/install.sh -->'
HAD_FAILURE=0
  ...
  case "$SCOPE" in
    project)
      case "$agent" in
        ...
        cursor|codebuddy|codex) echo skip ;;
      esac ;;
    user)
      case "$agent" in
...
install_agents_md_one() {
  ...
  case "$method" in
    ...
    mdc)
      {
        printf -- '---\ndescription: Project-wide agent guidelines installed from agent-plugins\nalwaysApply: true\n---\n'
        printf '%s\n\n' "$MARKER"
        cat "$src"
      } > "$target" || return 1
...
Options:
  ...
  --agents LIST          Comma-separated subset of: claude,cursor,codebuddy,codex
                         (default: all four)
...
Notes:
  - Project-scope AGENTS.md: only Claude Code needs a bridge (CLAUDE.md -> AGENTS.md);
    Cursor, CodeBuddy, and Codex read ./AGENTS.md natively.
